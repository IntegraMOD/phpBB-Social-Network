...
$foe    = (is_array($row) && isset($row['foe']))    ? (bool) $row['foe']    : false;
$friend = (is_array($row) && isset($row['friend'])) ? (bool) $row['friend'] : false;
...
if ($config['load_onlinetrack'])
{
	...
	$member['session_time']       = (is_array($session) && isset($session['session_time'])) ? $session['session_time'] : 0;
	$member['session_viewonline'] = (is_array($session) && isset($session['session_viewonline'])) ? $session['session_viewonline'] : 0;
	unset($session);
...
while ($relation = $db->sql_fetchrow($result))
{
	...
	else
	{
		...
		{
            $relationship_arr = (isset($relation['anniversary']) && is_string($relation['anniversary'])) ? array_map('intval', explode('-', $relation['anniversary'])): [];

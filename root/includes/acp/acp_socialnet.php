...
class acp_socialnet extends AddOnsHookSystem
{
    ...
    public $u_action;
    public $page_title = '';
    public $acpPanel_title = '';
    public $acpPanel_explain = '';
    public $motivationPicture = '';
    public $new_config = array();
    public $initModuleCacheName = '_sn_module_';
    public $initModuleMaxTime = 20;
    ...
    public function main($id, $mode)
    {
        ...
        if (preg_match('/^module_(.+)/i', (string) $mode, $module_match)) {
            $call_module = 'module';
        ...
        if ($this->acpPanel_title != '') {
            ...
            $this->page_title = $user->lang[$this->page_title] . ' &bull; ' . trim(preg_replace('/' . $user->lang['SETTINGS'] . '/si', '', (string) $this->acpPanel_title));
        }
     ...
     */
    public function module($id, $module)
    {
    ...
    public function sett_main($id)
    {
        ...
        for ($i = 0; $i < count($modules) && isset($modules[$i]); $i++) {
            $module = preg_replace('/^module_/si', '', (string) $modules[$i]['module_mode']);
     ...
     * @param integer $id phpBB variable
     */
    public function sett_modules($id)
    {
        ...
        while ($row = $db->sql_fetchrow($rs)) {
            ...
            $CONFIG_NAME = strtoupper((string) $row['config_name']);
        ...
        if ($continue == 0) {
            ...
            foreach ($cfg_array as $module => $enabled) {
                ...
                if ($module_current[$module] != $enabled) {
                    $acp_moduleclass = 'acp_' . preg_replace('/^module_/si', '', (string) $module);
        ...
        if ($continue == 1) {
            ...
            if ($continue == 1) {
                trigger_error($user->lang['CONFIG_UPDATED'] . str_replace('&amp;continue=1', '', (string) adm_back_link($this->u_action)));
            }
        }
     ...
     * @param integer $id phpBB variable
     */
    public function sett_confirmBox($id)
    {
        global $user;
    ...
    public function blocks_enable($id)
    {
        global $user, $config;
    ...
    public function blocks_config($id, $block)
    {
        ...
        foreach ($avail_blocks as $block_name => $enable) {
            $blockName = substr((string) $block_name, 3);
     ...
     */
    public function _settings(mixed $id, $mode, $display_vars = array())
    {
    ...
    public function _array_filter_key($input, $callback)
    {
        if (!is_array($input)) {
     ...
     * @return void
     */
    public function _set_config($config_name, mixed $config_value, $is_dynamic = false)
    {
...
{
    $filter = '/^sn_block/si';
    return preg_match($filter, (string) $elem);
}
...
{
    public $aoh_mode = 'addons';
    public $aoh_sub = '';
    public $aoh_u_action = '';

    public $aoh_tabs = array(
        array(
            'sub'		 => 'addon',
    ...
    public function sub_addon($id, &$error)
    {
    ...
    public function sub_editaddon($id, &$error)
    {
        ...
        if ($ad_id == 0) {
            $this->aoh_tabs[2]['name'] = 'SN_ADDONS_ADDADDON';
            //print_r( $this->aoh_tabs);
            //			$template->alter_block_array('sn_tabs', array('NAME' => ), true, 'change');
            $s_add = '';
        } else {
    ...
    public function sub_moveaddon($id, &$error)
    {
        global $db, $user;
    ...
    public function sub_enableaddon($id, &$error)
    {
        global $db;
    ...
    public function sub_deleteaddon($id, &$error)
    {
        global $db, $user;
    ...
    public function sub_placeholder($id, &$error)
    {
    ...
    public function sub_editplaceholder($id, &$error)
    {
        global $template, $db, $user;
        ...
        if ($ph_id == 0) {
            $this->aoh_tabs[3]['name'] = 'SN_ADDONS_ADDHOLDER';
        } else {
    ...
    public function sub_deleteplaceholder($id, &$error)
    {
        global $db, $user;
    ...
    public function _available_placeholders($ph_id = 0)
    {
        global $template, $db, $user;
    ...
    public function _available_addons($s_add = '')
    {

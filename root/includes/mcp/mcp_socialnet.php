...
class mcp_socialnet
{
    ...
    public function main($id, $mode)
    {
        ...
        $this->tpl_name = 'socialnet/mcp_socialnet';
        ...
        if (!preg_match('/^module_([^_]+)_?(.+)?$/i', (string) $mode, $module_match)) {
        ...
        $module_mcp_filename = "{$socialnet_root_path}mcp/mcp_{$module}.{$phpEx}";

        if (!file_exists($module_mcp_filename)) {
            trigger_error(sprintf($user->lang['SN_FILE_NOT_EXISTS'], $module_mcp_filename));
        }

        include_once($module_mcp_filename);
        $mcp_module = 'mcp_' . $module;
        if (!class_exists($mcp_module) || !method_exists($mcp_module, 'main')) {
            trigger_error(sprintf($user->lang['SN_MODULE_NOT_EXISTS'], $module.'::main()', $module_mcp_filename));
        }

        $modul = new $mcp_module($this);
    ...
    public function _settings(mixed $id, $mode, $display_vars = array())
    {
        ...
        $form_key = 'mcp_' . $mode . '_settings';
        add_form_key($form_key);

        $submit = (isset($_POST['submit'])) ? true : false;

        $display_vars = array_merge(array('title' => 'MCP_IM_SETTINGS'), $display_vars);
     ...
     * @return void
     */
    public function _set_config($config_name, mixed $config_value)
    {
        global $db, $user;
...
function validate_config_vars($config_vars, &$cfg_array, &$error)
{
    ...
    foreach ($config_vars as $config_name => $config_definition) {
        ...
        $validator = explode(':', (string) $config_definition['validate']);

        // Validate a bit. ;) (0 = type, 1 = min, 2= max)
        switch ($validator[$type]) {
            case 'string':
                $length = strlen((string) $cfg_array[$config_name]);
            ...
            case 'script_path':
                ...
                $destination = str_replace('\\', '/', (string) $cfg_array[$config_name]);
                ...
                $cfg_array[$config_name] = trim((string) $destination);
            ...
            case 'lang':
                ...
                $cfg_array[$config_name] = basename((string) $cfg_array[$config_name]);
            ...
            case 'wpath':
                ...
                $cfg_array[$config_name] = trim((string) $cfg_array[$config_name]);

                // Make sure no NUL byte is present...
                if (str_contains($cfg_array[$config_name], "\0") || str_contains($cfg_array[$config_name], '%00')) {
...
function validate_range($value_ary, &$error)
{
    ...
    foreach ($value_ary as $value) {
        $column = explode(':', (string) $value['column_type']);
        $max = $min = 0;
        $type = 0;
        ...
        switch ($type['php_type']) {
            case 'string':
                ...
                if (strlen((string) $value['value']) > $max) {
